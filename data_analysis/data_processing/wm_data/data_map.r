...
am <- dbReadTable(con, "AlmaMater.csv")
am_names <- gsub('[^a-zA-Z ]', '', am$Executive)

am_name_parts <- strsplit(am_names, " ", fixed=F)

# for(i in length(am_name_parts)){
#   for(j in length(hni_name_parts)){
...
tmp_matches <- rep(0, length(am_name_parts))
for(i in 1:length(am_name_parts)){
  tmp_flags <- rep(0, length(hni_name_parts))
  for(j in 1:length(am_name_parts[[i]])){
    tmp_ids <- hni_name_id[hni_name_parts_ul == am_name_parts[[i]][j]]
