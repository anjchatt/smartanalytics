...
for(i in 1:n){
  ...
  for(age in 18:80){
    ...
    stop_flg <- runif(1) < p_iter_stop[age]
    if(stop_flg){
      break
    }
  ...
  for(age in 1:j){
    ...
    fp <- fp/(1+discount_r) + customers[j_,products_p]
  ...
  if(i==1){
    ...
    write.table(customers[customers_id,], file = file_name, append = F, quote = TRUE, sep = ",",
                eol = "\n", na = "NA", dec = ".", row.names = F,
                col.names = TRUE, qmethod = c("escape", "double"),
                fileEncoding = "")
  }else{
    customers[,logical_id] <- customers[,logical_id]+0
    write.table(customers[customers_id,], file = file_name, append = TRUE, quote = TRUE, sep = ",",
                eol = "\n", na = "NA", dec = ".", row.names = F,
