...
f_p_cc <- function(CC, inc, pCC, ypCC){
  if(CC){
    if(ypCC == 1){
      profit <- round(inc/3*(3+rnorm(1)/1))/50
    }else{
      profit <- round(inc/10*(1+rnorm(1)/1))/200
    }
  }else{
    profit <- 0
  }
...
f_real_est <- function(Edu_, Age, REStat_, Mar_, mortg){
  ...
  if(REStat_ == 'Lives_with_relatives'){
    age_move_prob <- c(0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.03, 0.1, 0.1, 1)
...
f_ch_num <- function(ChNum_, mar, age){
  if(!mar){
    ...
    probs <- c(0.3, 0.2, 0.1, 0.02, 0.02, 0.02, 0.02, 0)
    prob <- probs[ChNum_+1]
    if(age>50){
      prob <- 0
    }else if(age>45){
...
f_soc_stat <- function(edu, age, soc_st){
  if(!edu){
    if(age > 26){
      stat <- "Employed"
    }else{
...
f_income_evol <- function(age, edu, inc_, guy_trend){
  ...
  if(edu){
    mean_speed <- mean_speed + 3e3
  }
  
  inc <- inc_ + mean_speed*guy_trend + round(rnorm(1)*2)*1000
  if (inc<0){
    inc <- 0
  }else if(inc<llim){
    inc <- inc_ + abs(round(rnorm(1)*2)*500)
  }else if(inc>ulim*guy_trend){
    inc <- inc_ - abs(round(rnorm(1)*2)*500)
  }
  inc
}
